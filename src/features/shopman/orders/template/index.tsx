...
export const PedidosTemplate = () => {
  const { orders } = ORDERS_DATA
  const [searchTerm, setSearchTerm] = useState("")
  const [activeFilter, setActiveFilter] = useState<OrdersFilter>("Todos")
  const [expandedOrders, setExpandedOrders] = useState<string[]>([])
  const [repeatedOrders, setRepeatedOrders] = useState<string[]>([])

  const filterMap: Record<OrdersFilter, OrderStatus[] | null> = {
    Todos: null,
  ...
  const navItems = [
    { id: "catalog", label: "Catalogo", icon: LayoutGrid, href: "/shopman/catalog" },
    {
      id: "cart",
      label: "Carrito",
      icon: ShoppingCart,
